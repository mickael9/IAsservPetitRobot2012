...
/*
 * Fonction de mise a jour... POUR LE&#65533;PETIT ROBOT, RENVOIE 1 SI LA CONSIGNE EST TERMINEE
 */
 
 
bool CommandManager::perform() {
  ...
  */
  ...
  if ( !cnsgCtrl->areRampsFinished() ) { 
    ...
    if (currCMD.type == CMD_GO || currCMD.type == CMD_TURN ) { // On avance ou on tourne sur place
      return res; //Dans ce cas, on attend simplement d'etre arrive :)
...
void CommandManager::calageBordurePetit(int sens) {
    ...
    pc.printf("Debut calage bordure\n");
  cnsgCtrl->setLowSpeed(true);
    
  // On recule 2sec
  cnsgCtrl->add_dist_consigne(Utils::mmToUO(odometrie,-100));
  wait(2);
  cnsgCtrl->angle_Regu_Off(); //on coupe le r&#65533;gu d'angle pour s'aligner avec la bordure
  ...
  wait(1);
    ...
    pc.printf("Retour a la case depart\n");
  ...
  cnsgCtrl->add_dist_consigne(Utils::mmToUO(odometrie,-30));
  wait(3);
